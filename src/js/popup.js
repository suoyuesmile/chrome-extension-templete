...
function sendRequest() {
  ...
  bg.getData('http://172.16.6.68:8765', function(text) {
    console.log(text)
    $.ajax({
      url: 'http://youdun.ctools.cn/text',
...
// 3.渲染页面
function penderPopup(data) {
  // var items = [
  //  	{
  // 		sense_sentence: "百万级线索及标签",
  // 		sense_type: "新广告法敏感词",
  // 		sense_word: "营销",
  // 	}, {
  // 		sense_sentence: "百万级线索及标签",
  ...
  handlePageWord(items)
}

// 4.点击后处理DOM高亮文字
function handlePageWord(items) {
  sendMessageToContentScript({ cmd: 'posWord', items: items }, function(
    response
  ) {
    var items = response
    // console.log(items);
    $('.sense_count').text(items.length)
    for (var i = 0; i < items.length; i++) {
      var length = items[i].word.length
      var start = items[i].sentence.indexOf(items[i].word)
      var end = start + length
      var sentence = $('<div class="sentence"></div>')
      var word = $('<div class="word"></div>')
      var item = $('<div id="' + items[i].id + '" class="item"></div>')
      sentence.html(
        items[i].sentence.substring(0, start) +
          '<span class="sense_word">' +
          items[i].word +
          '</span>' +
          items[i].sentence.substring(end)
      )
      word.html('敏感词类型：' + items[i].label)
...
$(document).on('click', '.item', function(e) {
  var id = $(this).attr('id')
  // alert(1);
  sendMessageToContentScript({ cmd: 'posPage', id: id }, function() {})
})
// 5.定位敏感词
...
$('#send_message_to_content_script').click(() => {
  sendMessageToContentScript('你好，我是33！', response => {
